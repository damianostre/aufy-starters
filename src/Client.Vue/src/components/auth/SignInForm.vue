<template>
    <form class="space-y-6" action="#" method="POST" @submit.prevent="handleSubmit(onSubmit)" novalidate>
        ...
        <div>
            ...
            <p class="mt-2 text-sm text-red-600" id="email-error">{{ errors.email?.message }}</p>
        </div>
        ...
        <div>
            ...
            <p class="mt-2 text-sm text-red-600" id="password-error">{{ errors.password?.message }}</p>
        </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="text-sm leading-6">
                <RouterLink to="/forgot-password"
                            class="font-semibold primary-color hover:primary-color">Forgot password?</RouterLink>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between">
            <div class="text-sm leading-6">
                ...
                <RouterLink to="/signup"
                            class="ml-3 font-semibold primary-color hover:primary-color">Sign up now</RouterLink>
            </div>
        </div>
    </form>
</template>
...
const onSubmit = async (data: FormModel) => {
    try {
        await aufy.signIn({ email: data.email, password: data.password });
        navigate('/');
    } catch (error) {
